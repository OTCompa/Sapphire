...
void ActionResult::applyStatusEffect( uint32_t id, int32_t duration, Entity::Chara& source, uint8_t param,
                                      const std::vector< StatusModifier >& modifiers, uint32_t flag, bool shouldOverride, float critProbability, float critBonus, float tickValue )
{
  m_result.Value = static_cast< int16_t >( id );
  m_result.Arg2 = param;
  m_result.Type = CalcResultType::TypeSetStatus;
  ...
  if (tickValue > -1) {
    for( const auto& [ modifier, _] : m_pStatus->getStatusModifiers() )
    {
      if (modifier == ParamModifier::TickDamage || modifier == ParamModifier::TickHeal) {
        m_pStatus->registerTickEffect( modifier, tickValue );
      }
    }
  }
}
